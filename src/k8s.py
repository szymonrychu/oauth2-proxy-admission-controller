...
def get_config(secret_name:str = None, secret_namespace:str = None) -> Optional[Config]:
    ...
    if default_config_secret_name:
        raw_configuration = _kd_client.secrets.get(default_config_secret_name, default_config_secret_namespace).data
...
def generate_proxy_container(upstream_port_number:int, port_name:str, config:Config) -> kd.models.V1Container:
    ...
    new_container.env = [
        ...
        kd.models.V1EnvVar(name='OAUTH2_PROXY_UPSTREAMS', value=f"http://127.0.0.1:{upstream_port_number}"),
    ...
    new_container.livenessProbe = kd.models.V1Probe(failureThreshold=3, periodSeconds=10, successThreshold=1, tcpSocket=kd.models.V1TCPSocketAction(port=port_name))
    new_container.readinessProbe = kd.models.V1Probe(failureThreshold=3, periodSeconds=10, successThreshold=1, tcpSocket=kd.models.V1TCPSocketAction(port=port_name))
...
def find_pods_services(pod:kd.models.V1Pod, port:kd.models.V1ContainerPort) -> List[kd.models.V1Service]:
    ...
    for starting_point in range(labels_num):
        ...
        for c in range(labels_num):
            if selector:
                ...
                services =  _kd_client.services.find(pattern='.*', namespace=pod.metadata.namespace, label_selector=_full_selector)
...
def update_services(service:kd.models.V1Service, container_port:kd.models.V1ContainerPort, pod:kd.models.V1Pod):
...
def revert_services(pod:kd.models.V1Pod):
    services = _kd_client.services.find(pattern='.*', namespace=pod.metadata.namespace, label_selector=f"{SERVICE_ANNOTATION_TYPE_KEY}={SERVICE_ANNOTATION_TYPE_VALUE_INSECURE}")
    ...
    if insecure_service:
        _kd_client.services.delete(insecure_service.metadata.name, insecure_service.metadata.namespace)
